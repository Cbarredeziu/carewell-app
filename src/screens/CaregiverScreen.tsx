...
const CaregiverScreen = () => {
  ...
  const [summary, setSummary] = useState<CaregiverSummary>({ date: new Date().toDateString(), adherencePercent: 82, recentSymptoms: [] });
  ...
  useEffect(() => {
    loadSymptoms()
      .then((symptoms: SymptomEntry[]) => {
        ...
        setSummary(prev => ({ ...prev, recentSymptoms: symptoms.slice(0, 3) }));
      })
      .catch(console.error);
  }, []);
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={styles.headerRow}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Caregiver summary</Text>
        <TouchableOpacity
          style={[styles.secondaryBtn, { borderColor: theme.colors.border }]}
          onPress={() => setIsAdding(true)}
        >
          <Text style={[styles.secondaryText, { color: theme.colors.text }]}>Log symptom</Text>
        </TouchableOpacity>
      </View>
      <View style={[styles.card, { backgroundColor: theme.colors.card }]}> 
        <Text style={[styles.label, { color: theme.colors.text }]}>Adherence</Text>
        <Text style={[styles.value, { color: theme.colors.text }]}>{summary.adherencePercent}% today</Text>
        <Text style={[styles.label, { color: theme.colors.text, marginTop: 12 }]}>Recent symptoms</Text>
        {summary.recentSymptoms.length === 0 ? (
          <Text style={{ color: theme.colors.muted }}>No symptoms logged today.</Text>
        ) : (
      ...
      <Text style={[styles.note, { color: theme.colors.muted }]}>Sharing is local-only in MVP; no cloud sync yet.</Text>
                ...
                <TouchableOpacity
                  ...
                  style={[
                    styles.severityChip,
                    {
                      ...
                      backgroundColor: severity === level ? theme.colors.primary : 'transparent'
                    }
                  ]}
